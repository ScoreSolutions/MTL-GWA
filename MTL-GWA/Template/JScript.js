...
function ValidPassword(txtPwd) {
    var txtPwd1 = document.getElementById(txtPwd);
    if (txtPwd1.value != "") {
        if (txtPwd1.value.length < 6) {
            alert("รหัสผ่านจะต้องไม่น้อยกว่า 6 ตัวอักษร \r\n( Password is contains at least 6 characters. )");
            txtPwd1.select();
        } else {
            return ValidPasswordBlur(event, txtPwd);
        }
    }
}

function ValidPasswordBlur(e, txtPwd) {
    var txtPwd = document.getElementById(txtPwd);
    var evt = e ? e : window.event;
    var zz = evt.keyCode || evt.charCode;
    //alert(zz);
...
//                e.preventDefault();
            ...
            for (i = 0; i < str.length;i++ ){
                ...
                if (str.substring(i - 1, i) == d && str.substring(i - 2,i-1) == d) {
                    ret = false;
                    break;
                }
            }
...
function ValidPwdKeyUp(e, txtPwd) {
    //รหัสผ่านตัวแรกจะต้องเป็นตัวอักษรเท่านั้น
    var txtPwd = document.getElementById(txtPwd);
    if (txtPwd.value.length != 0) {
        var yy = txtPwd.value.charCodeAt(0);
        if ((yy >= 65 && yy <= 90) || (yy >= 97 && yy <= 122)) {
            null;
        } else {
            txtPwd.value = txtPwd .value.substring(1);
            alert('ตัวอักษรตัวแรกจะต้องเป็นตัวอักษรภาษาอังกฤษเท่านั้น \r\n ( The first charactrs must be in English letter. )');
            if (window.event)
                event.returnValue = false;
            else
